...
class EntryRepository {
  ...
  Future<List<Map<String, dynamic>>> getAllEntries() async {
    final db = await DatabaseInstance.instance.db;
    final res = await db.query("entries", orderBy: "timestamp DESC");
    return res;
  }
  ...
  Future<int> insertEntry(
    ...
    final db = await DatabaseInstance.instance.db;
    final data = {"prompt": prompt, "timestamp": timestamp, "content": content};
  ...
  Future<int> updateEntry(int id, Map<String, dynamic> data) async {
    final db = await DatabaseInstance.instance.db;
    return db.update("entries", data, where: "id = ?", whereArgs: [id]);
  }
}
